...
from .utils import tinhngaydauky, laychisongay, laydientieuthungay, \
    ...
    laylichcatdien, laychisongaygannhat, lay_tien_no_evn, luu_ky_hien_tai
...
class EVNSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator, customer_id, sensor_type, unique_id, unit=None, ngaydauky=1):
        ...
        super().__init__(coordinator)  # type: ignore
    ...
    def state(self):
        ...
        if self._sensor_type == "chi_so_cuoi_ky":
            ...
            if chi_so is None or chi_so <= 0:
                ...
                if chi_so_gan_nhat is not None:
                    # Check value > 0 safely
                    val = 0.0
                    try:
                        val = float(chi_so_gan_nhat)
        ...
        if self._sensor_type == "tieu_thu_ky_nay":
            ...
            if tam_chot_state is not None and cuoi_ky_state is not None:
                try:
                    ...
                    if chi_so_tam_chot > 0 and chi_so_cuoi_ky > 0 and chi_so_tam_chot > chi_so_cuoi_ky:
                        ...
                        # Lưu vào DB để API WebUI lấy đồng bộ
                        luu_ky_hien_tai(self._userevn, tieu_thu, 0, start_ky.strftime("%d-%m-%Y"))
            ...
            if chi_so_tam_chot is not None and chi_so_cuoi_ky is not None:
                if chi_so_tam_chot > 0 and chi_so_cuoi_ky > 0 and chi_so_tam_chot > chi_so_cuoi_ky:
                    ...
                    # Lưu vào DB
                    luu_ky_hien_tai(self._userevn, tieu_thu, 0, ngay_bat_dau_ky)
        ...
        if self._sensor_type == "tien_dien_ky_nay":
            ...
            if tieu_thu_state is not None and tieu_thu_state.state not in ('unknown', 'unavailable', '0', '0.0'):
                try:
                    ...
                    if tieu_thu > 0:
                        ...
                        # Lưu vào DB cả tiêu thụ và tiền điện
                        ngay_bat_dau = self._attributes.get("Ngày bắt đầu", "")
                        luu_ky_hien_tai(self._userevn, tieu_thu, int(round(float(tongtien))), ngay_bat_dau)
                        
                        return int(round(float(tongtien))) if tongtien is not None else 0
                except (ValueError, TypeError):
                    pass
            ...
            if chi_so_tam_chot is not None and chi_so_cuoi_ky is not None:
                if chi_so_tam_chot > 0 and chi_so_cuoi_ky > 0 and chi_so_tam_chot > chi_so_cuoi_ky:
                    ...
                    # Lưu vào DB
                    luu_ky_hien_tai(self._userevn, tieu_thu, int(round(float(tongtien))), ngay_bat_dau_ky)
        ...
        if self._sensor_type == "tieu_thu_ky_truoc":
            # Tính tháng kỳ trước dựa trên ngaydauky
            if self._ngaydauky == 1:
                # Kỳ theo tháng dương: kỳ trước = tháng trước
                ...
                # Nếu hôm nay < ngaydauky, đang ở kỳ hiện tại
                # Kỳ trước kết thúc vào tháng trước
                if today.day < self._ngaydauky:
                    ...
                    # Nếu hôm nay >= ngaydauky, đang ở kỳ sau
                    # Kỳ trước kết thúc vào tháng hiện tại
                    thang = today.month
                    nam = today.year
        ...
        if self._sensor_type == "chi_tiet_dien_tieu_thu_ky_nay":
            ...
            if len(all_dates) > 0:
                ...
                start_db = last_date.strftime("%d-%m-%Y")
                end_db = first_date.strftime("%d-%m-%Y")
            ...
            if lich_cat_dien:
                ...
                for event in lich_cat_dien:
                    ...
                    try:
                        ...
                        _LOGGER.debug(f"Error parsing date '{event.get('Ngày')}' in lich_cat_dien: {e}")
                        continue
